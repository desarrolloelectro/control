...
function change_medio_pago(mediopago){
    ...
    function asociarProductoAnticipo(producto,anticipo,mensaje=""){
        <?php
        /**
        ...
        var d = '{{ $dia_iva->id }}';
        var params = '?i='+i+'&d='+d+'&p='+producto+'&a='+anticipo+'&c_token='+c_token+'&_token='+_token;

        var myRequest = new Request('{{ route('dsi.asociateProductAdvance') }}'+params, myInit);
        ...
        var loading = document.getElementById('loading');
            ...
            if(contentType && contentType.indexOf("application/json") !== -1) {
                ...
                json.data.forEach(obj => {
                    ...
                    contenidoTablaAdvancesProductos.innerHTML += `<tr data-dismiss="modal" title="Haga clic para seleccionar este producto" onclick="rellenarProducto('${obj.nombre}', '${obj.referencia}', '${obj.valor}', '${obj.linea}');">
                    <td>${obj.num_recibo}</td>
                    <td>${obj.fecha_recibo}</td>
                    <td style="text-align:right;">$ ${ new Intl.NumberFormat("de-DE").format(obj.valor_recibo) }</td>
                    <td>
                        <button type="button" onclick="asociarProductoAnticipo(document.getElementById('id_producto_para_anticipo').value,${obj.id},'Confirma que desea utilizar el anticipo de $ ${ new Intl.NumberFormat("de-DE").format(obj.valor_recibo) } en el producto con referencia ${document.getElementById('ref_producto_para_anticipo').value}?')" class="btn btn-success">Seleccionar</button>
                    </td>
                    <tr>`;
                });           
